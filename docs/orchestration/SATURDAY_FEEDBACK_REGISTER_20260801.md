...
| SAT-018 | Homepage clean | Comparison controls | major | rendered inspection found CSS-generated `pending` labels on two disabled comparison tabs that text-only checks missed; the clean builder now removes unavailable tabs and the checker fails if disabled comparison controls return | clean-unfinished-signal-removed | Codex | preserve in derived rebuilds | accepted |
...
| SAT-020 | Homepage | Benefit-strip wording | major | Carli's Canva preview says `No Caffeine, Organic Ingredients & Vegan Friendly`; the clean Homepage currently uses the existing four-part `Naturally sweet / No caffeine / Nothing added / Organic & vegan` strip, so the exact replacement and placement have not been silently inferred | exact-copy-choice-held | Nate/Carli | approve the exact public strip before WIP or production application | needs_review |
