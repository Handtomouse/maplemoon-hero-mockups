...
/** MapleMoon homepage style-finish R2: four bounded CSS corrections only. */
...
const PACKET_ID = "MAPLEMOON-HOMEPAGE-STYLE-FINISH-R2-20260825T163529";
const OUTPUT = path.join(REPO, "_wip/deploy/generated/maplemoon-homepage-style-finish-r2-20260825T163529");
...
const CSS_RELATIVE = "styles/homepage-style-finish-r2.css";
const MANIFEST_RELATIVE = "homepage-style-finish-r2-manifest.json";
const LINK = '<link rel="stylesheet" href="/styles/homepage-style-finish-r2.css" data-maplemoon-homepage-style-finish-r2="20260825T163529">';
...
const FILE_PINS = new Map([
  ...
  [path.join(REPO, "docs/orchestration/packets/MAPLEMOON-HOMEPAGE-STYLE-FINISH-20260825T161314.md"), "16b627382d3c6b94eea3b10218335387882d71fe4c67637e4e9b6c6a6cf896c4"],
  [path.join(REPO, "docs/orchestration/reviews/MAPLEMOON-HOMEPAGE-STYLE-FINISH-20260825T161314.json"), "d345e2461148b041aa660bc9f0f399808d07b07cefba7d338eb31db358138606"],
...
const R2_CORRECTIONS = `

/* R2 bounded corrections: no change outside the four failed contracts. */

/* R2-01 — the two governed hotspot labels remain >=14px through 320px. */
...
/* R2-02 — the existing single farm-credit text node remains >=14px / 1.4. */
html body .wf .wf-where1 .wf-photo-credit {
  font-size: 14px !important;
  line-height: 1.45 !important;
}

/* R2-03 — editorial comparison stacks in DOM order at 768 and narrower. */
...
/* R2-04 — bind the real ID and remove the inherited pale pseudo bridge. */
...
function main() {
  ...
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "maplemoon-homepage-style-finish-r2-"));
  ...
  try {
    ...
    if (baselineHomepage.includes(CSS_RELATIVE) || baselineHomepage.includes("data-maplemoon-homepage-style-finish-r2")) fail("R2 style link already exists in baseline");
    const derivedHomepage = baselineHomepage.replace("</head>", `${LINK}\n</head>`);
    if ((derivedHomepage.match(/data-maplemoon-homepage-style-finish-r2=/g) || []).length !== 1) fail("R2 link injection is not count-one");
    ...
    const r2Style = `${r1Style.trimEnd()}${R2_CORRECTIONS}`;
    const cssPath = path.join(STAGING, CSS_RELATIVE);
    fs.mkdirSync(path.dirname(cssPath), { recursive: true });
    fs.writeFileSync(cssPath, r2Style, "utf8");

    const dom = strippedDom(derivedHomepage);
    if (/<[^>]+class=["'][^"']*\bq-segments\b/i.test(dom)) fail("comparison segment control exists in R2 DOM");
    ...
    const manifest = {
      schema: "maplemoon-homepage-style-finish-r2/v1",
      packet_id: PACKET_ID,
      disposition: "BOSS_REVIEW_ONLY_NOT_PROMOTED",
      created_at: "2026-08-25T16:35:29+10:00",
      predecessor: {
        packet_id: r1Manifest.packet_id,
        output_directory_sha256: R1_DIRECTORY_SHA256,
      ...
      mutation_surface: {
        homepage: "count-one R2 stylesheet link injection only",
        stylesheet: `/${CSS_RELATIVE}`,
        manifest: `/${MANIFEST_RELATIVE}`,
        non_home_and_support_files: "byte-identical to fresh pinned baseline",
      },
      corrections: [
        { id: "R2-01", contract: "hotspot font >=14px through 320", selector: ".wf-what1 .co and text leaves" },
        { id: "R2-02", contract: "credit font >=14px and line-height >=1.4", selector: ".wf-where1 .wf-photo-credit" },
        { id: "R2-03", contract: "comparison one column at <=768", selector: ".q-compare-pro" },
        { id: "R2-04", contract: "real sampler ID resolves dark and inherited pseudo is absent", selector: "#sampler.q-sampler and ::before" },
      ],
    ...
    console.log(`BUILD PASS packet=${PACKET_ID} output=${OUTPUT} baseline_files=${baselineFiles.length} output_files=${outputFiles.length} bytes=${outputBytes} link_injections=1 reverse_equal=1 q_segments_dom=0 non_home_equal=1 r1_preserved=1 corrections=4/4`);
  } finally {
