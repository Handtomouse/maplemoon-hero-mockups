...
(async () => {
  ...
  const report = {
    schema: "maplemoon-cart-inert-focus-regression/v1",
  ...
  fs.writeFileSync(path.join(OUT, "results", "cart-regression-audit.json"), JSON.stringify(report, null, 2) + "\n");
  ...
  fs.mkdirSync(path.join(OUT, "results"), { recursive: true });
  fs.writeFileSync(path.join(OUT, "results", "cart-regression-audit.json"), JSON.stringify(failure, null, 2) + "\n");
