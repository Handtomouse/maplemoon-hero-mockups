#!/usr/bin/env python3
"""Replay protected pins and browser-source parity for the isolated R3 recovery."""
...
R3_LANE = REPO / "_wip/evidence/design_system_gap_audit_close_r3_20260817T181126"
ORIGINAL_BROWSER = REPO / "_wip/evidence/design_system_gap_audit_20260817T153140/verify-current-site-browser.mjs"
R3_BROWSER = R3_LANE / "verify-current-site-browser-r3.mjs"
OLD_OUT = "const OUT = '/Users/handtomouse/maplemoon-website/_wip/evidence/design_system_gap_audit_20260817T153140';"
NEW_OUT = "const OUT = '/Users/handtomouse/maplemoon-website/_wip/evidence/design_system_gap_audit_close_r3_20260817T181126';"
OLD_PORT = "const PORT = 8798;"
NEW_PORT = "const PORT = 8799;"

PINNED_FILES = {
    REPO / "docs/orchestration/packets/MAPLEMOON-DESIGN-SYSTEM-GAP-AUDIT-CLOSE-R3-20260817T181126.md": "a5698ba48f8e15a63800bb29755412d610e21b2c66b4b7ce935ec3b961b5f659",
    ...
    R2_LANE / "verify-design-gap-audit.py": "7d90eb28d8875fc30cc5fe4a096a292fce57d827757063038eae9778d23d3363",
    R2_LANE / "verify-readonly-close.py": "e69bf199a7a5263f745e0e209286ad6c7d758cc2dc63a304c673000eea32f9d6",
    ...
    REPO / "_wip/evidence/design_system_gap_audit_20260817T153140/VISUAL-QA.json": "003fc78930f5bbb638d9359be32c21d28985bb1e7e7392381d0c93cca2ae5b8a",
...
PINNED_DIRECTORIES = {
    REPO / "_wip/evidence/design_system_gap_audit_20260817T153140": "7528133f77269d3168568796a84f863d9a9cf2f206d3b4b24a2ec13cbf0d56aa",
...
def validate_browser_parity() -> tuple[list[str], int]:
    errors: list[str] = []
    try:
        original = ORIGINAL_BROWSER.read_text(encoding="utf-8")
        recovered = R3_BROWSER.read_text(encoding="utf-8")
    except OSError as error:
        return [f"browser parity source unavailable: {error}"], -1

    if original.count(OLD_OUT) != 1:
        errors.append(f"original browser OUT substitution count is {original.count(OLD_OUT)}, expected 1")
    if original.count(OLD_PORT) != 1:
        errors.append(f"original browser PORT substitution count is {original.count(OLD_PORT)}, expected 1")
    if errors:
        return errors, -1

    expected = original.replace(OLD_OUT, NEW_OUT).replace(OLD_PORT, NEW_PORT)
    if recovered != expected:
        errors.append("R3 browser source is not byte-for-byte normalized parity after OUT/PORT substitution")

    diff = list(
        difflib.unified_diff(
            original.splitlines(),
            recovered.splitlines(),
            fromfile="original",
            tofile="r3",
            lineterm="",
        )
    ...
    expected_removed = [f"-{OLD_OUT}", f"-{OLD_PORT}"]
    expected_added = [f"+{NEW_OUT}", f"+{NEW_PORT}"]
    if removed != expected_removed or added != expected_added:
        errors.append(
            "R3 browser normalized diff is not exactly two changed constants "
            f"removed={removed!r} added={added!r}"
        )
    return errors, len(removed)


def main() -> int:
    errors = validate_pins(PINNED_FILES, PINNED_DIRECTORIES)

    parity_errors, changed_lines = validate_browser_parity()
    errors.extend(parity_errors)

    browser_verifier = R3_LANE / "verify-current-site-browser-r3.mjs"
    design_verifier = R3_LANE / "verify-design-gap-audit-r3.py"
    close_verifier = R3_LANE / "verify-readonly-close-r3.py"
    for verifier in (browser_verifier, design_verifier, close_verifier):
        if not verifier.is_file() or verifier.stat().st_size == 0:
            errors.append(f"R3 verifier missing or blank: {verifier}")
    ...
    if errors:
        print("READONLY_CLOSE_R3 FAIL " + " | ".join(errors))
        print(f"BROWSER_PARITY {'FAIL' if parity_errors else 'PASS'} changed_constants=OUT,PORT changed_lines={changed_lines} port=8799")
    ...
    print(f"BROWSER_PARITY PASS changed_constants=OUT,PORT changed_lines={changed_lines} port=8799")
