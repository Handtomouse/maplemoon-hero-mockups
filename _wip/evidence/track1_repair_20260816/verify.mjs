...
const output = '/Users/handtomouse/maplemoon-website/_wip/evidence/track1_repair_20260816';
...
async function makeContact(widthLabel, rows) {
  ...
  const height = Math.ceil(rows.length / columns) * cellHeight + (Math.ceil(rows.length / columns) + 1) * gap;
  const composites = [];
...
for (const route of routes) {
  for (const [widthLabel, viewport] of viewports) {
    ...
    const response = await page.goto(`${origin}/${route}.html`, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await settle(page);
    ...
    if (route === 'homepage' && widthLabel === '390') {
      ...
      interactions.menu = await page.evaluate(() => {
        const links = [...document.querySelectorAll('[data-mm-primary-nav] a,[data-mm-utility-nav] a,[data-mm-utility-nav] .wf-currency')];
        return {
          ...
          rows: links.map(link => {
            ...
            return { text: link.textContent.trim(), width: Math.round(rect.width), height: Math.round(rect.height), hitOwn: hit === link || link.contains(hit) };
          }),
        };
      });
    ...
    if (route === 'homepage' && widthLabel === '390') {
      ...
      if (interactions.menu.rows.length !== 6 || interactions.menu.rows.some(row => row.width < 388 || row.height < 44 || !row.hitOwn)) failures.push('menu-rows');
      if (interactions.homeCart.dialogCount !== 1 || !interactions.homeCart.cartText) failures.push('homepage-cart');
    }
    if (route === 'shop' && (!/\$12\.95/.test(interactions.shopCart.after) || interactions.shopCart.count !== '1' || !/Cart, 1 item, subtotal \$12\.95/.test(interactions.shopCart.label || ''))) failures.push('shop-cart');
    if (route === 'faq' && (interactions.faq.count !== 1 || interactions.faq.expanded !== 'true' || !interactions.faq.visible)) failures.push('faq-open');
    if (route === 'stockists' && (interactions.skip.count !== 1 || interactions.skip.targetCount !== 1 || interactions.skip.mainTag !== 'MAIN')) failures.push('stockists-skip');
    const row = { route, width: widthLabel, httpStatus: response?.status() || 0, metrics: base, screenshot, interactions, ...errors, failures, result: failures.length ? 'FAIL' : 'PASS' };
    results.push(row);
...
await browser.close();
const result = { source: origin, contacts, results, failures: results.filter(row => row.result === 'FAIL') };
fs.writeFileSync(path.join(output, 'visual-results.json'), JSON.stringify(result, null, 2) + '\n');
console.log(`SUMMARY pass=${results.length - result.failures.length} fail=${result.failures.length} total=${results.length}`);
