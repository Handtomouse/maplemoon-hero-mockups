...
for (const [route, pathname] of routes) {
  for (const width of widths) {
    ...
    await page.screenshot({ path: path.join(evidenceRoot, `route-${route}-${width}.png`) });
    rows.push({
      route,
      pathname,
      width,
