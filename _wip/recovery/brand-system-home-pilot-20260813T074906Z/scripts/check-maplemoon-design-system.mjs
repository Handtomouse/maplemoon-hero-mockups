...
if (args.length !== 1 || !allowedFlags.has(args[0])) {
  finish(
    3,
    'ERROR configuration: choose exactly one explicit mode',
    ['usage: node scripts/check-maplemoon-design-system.mjs --contracts-only|--route-conformance']
  );
}
...
for (const exception of exceptions || []) {
  if (exception.status !== 'baseline_registered') failures.push(`${exception.id}: Phase 0 exception must be baseline_registered`);
  const route = routeById.get(exception.route);
  if (!route) {
    failures.push(`${exception.id}: unknown route ${exception.route}`);
    continue;
  }
  if (!exception.binding || exception.binding.kind !== 'baseline') failures.push(`${exception.id}: no baseline binding`);
...
if (mode === '--route-conformance') {
  ...
  for (const route of routes || []) {
    ...
    for (const requiredImport of requiredImports) {
      if (!html.includes(requiredImport)) failures.push(`${route.id}: missing shared import ${requiredImport}`);
    }
...
const routeState = mode === '--contracts-only' ? 'routeConformance=SKIPPED (flag not set)' : 'routeConformance=PASS';
