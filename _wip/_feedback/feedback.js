(function(){
  ...
  "#mm-fb-toggle{background:#1f2b38;color:#e7e4ca;border:1px solid #b3a380;border-radius:40px;padding:11px 16px;font:600 12px/1 sans-serif;letter-spacing:.03em;cursor:pointer;box-shadow:0 6px 20px rgba(0,0,0,.45)}",
  "#mm-fb-toggle.on{background:#b3a380;color:#1a1d16}",
  "#mm-fb-list-btn{background:#1f2b38;color:#e7e4ca;border:1px solid #33414f;border-radius:40px;padding:11px 14px;font:600 12px/1 sans-serif;cursor:pointer}",
  ...
  "#mm-fb-panel.open{display:block}#mm-fb-panel h4{margin:2px 4px 8px;font:700 12px/1 sans-serif;color:#e7e4ca}",
  ...
  bar.innerHTML='<button id="mm-fb-toggle">&#127991; Tag feedback</button><button id="mm-fb-list-btn">Tags <span id="mm-fb-count">0</span></button><span id="mm-fb-hint">Click an element, or <b>drag a box</b> for a region</span>';
  ...
  var toggle=bar.querySelector('#mm-fb-toggle'),listBtn=bar.querySelector('#mm-fb-list-btn'),hint=bar.querySelector('#mm-fb-hint');
  ...
  function renderPanel(){
    panel.innerHTML='<h4>Tags on this page ('+tags.length+')</h4>';
    tags.forEach(function(rec,i){
      var row=document.createElement('div');row.className='mm-fb-row';
      row.innerHTML='<span class=n>'+(i+1)+'</span><div><span class=c>'+rec.cat+(rec.region?' · region':'')+'</span><br>'+(rec.note||'(no note)').replace(/</g,'&lt;')+'<br><span style="color:#6b7580;font-size:10px">'+((rec.text||'').slice(0,40))+'</span></div><span class=x data-id="'+rec.id+'">&times;</span>';
  ...
  function openPop(info,x,y){
    ...
    pop.innerHTML='<div class="t">'+head+'</div><select id="mm-fb-cat">'+opts+'</select><textarea id="mm-fb-note" placeholder="What should change here?"></textarea><div class="btns"><button id="mm-fb-cancel">Cancel</button><button id="mm-fb-save">Save tag</button></div>';
    ...
    pop.querySelector('#mm-fb-save').onclick=function(){
      var cat=pop.querySelector('#mm-fb-cat').value,note=pop.querySelector('#mm-fb-note').value.trim();
      var rec={url:location.pathname,sel:info.sel,text:info.tx,cat:cat,note:note,rect:info.rect,vw:window.innerWidth,region:!!info.isRegion};
  ...
  fetch('/__feedback/log').then(function(r){return r.text();}).then(function(t){
    t.trim().split('\n').filter(Boolean).forEach(function(l){try{var r=JSON.parse(l);if(r.url===location.pathname){tags.push(r);addPin(r);}}catch(e){}});renderCount();renderPanel();});
})();
